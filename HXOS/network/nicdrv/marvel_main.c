...
//	unsigned long flags;
	...
	/* Wake main thread if commands are pending */
	 if (!priv->cur_cmd || priv->tx_pending_len > 0)//当前命令执行完成，或者有数据要发送，唤醒主线程处理
...
static int lbs_init_adapter(struct lbs_private *priv)//主要是初始化网卡的一些软件信息
{
	size_t bufsize;
	int i, ret = 0;
	...
	//分配一个服务集描述符的空间，用来存放这个BSS的一些基本信息
	...
	memset(priv->current_addr, 0xff, ETH_ALEN);//当前物理地址软件初始化为0xff

	priv->connect_status = LBS_DISCONNECTED;//未连接
	priv->mesh_connect_status = LBS_DISCONNECTED;//mesh网络未连接
	priv->secinfo.auth_mode = IW_AUTH_ALG_OPEN_SYSTEM;//认证模式，开放型认证
	priv->mode = IW_MODE_INFRA;//基础网络
	priv->curbssparams.channel = DEFAULT_AD_HOC_CHANNEL;//实际上就是效果最好的信道，信道6
	priv->mac_control = CMD_ACT_MAC_RX_ON | CMD_ACT_MAC_TX_ON;//接收发送使能
	priv->radio_on = 1;
	priv->enablehwauto = 1;
	...
	/*setup_timer(&priv->command_timer, command_timer_fn,//安装一个命令处理的定时器
		(unsigned long)priv);*/
	...
	init_waitqueue_head(&priv->cmd_pending);//初始化一个等待队列处理命令*/

	/* Allocate the command buffers */
	if (lbs_allocate_cmd_buffer(priv)) {//分配并初始化命令控制块struct cmd_ctrl_node是命令存放的主要结构
		lbs_pr_err("Out of memory allocating command buffers\n");
		ret = -ENOMEM;
		goto out;
	}
	priv->resp_idx = 0;
	priv->resp_len[0] = 0;//硬件返回的命令响应

	/* Create the event FIFO */
	/*priv->event_fifo = kfifo_alloc(sizeof(u32) * 16, GFP_KERNEL, NULL);//硬件的event，比如热插拔等事件
...
//	struct lbs_private *priv = dev->ml_priv;
	...
	do{
		...
		/*add_wait_queue(&priv->waitq, &wait);//将线程加入到等待队列，直到调用schedule()线程进入睡眠
		set_current_state(TASK_INTERRUPTIBLE);
		spin_lock_irq(&priv->driver_lock);

		if (kthread_should_stop())//终止
			shouldsleep = 0;	*/
		...
		//else if (priv->tx_pending_len > 0)//有数据需要发送
		//	shouldsleep = 0;	/* We've a packet to send */
		else if (priv->resp_len[priv->resp_idx])//响应，主要还是用于处理802.11管理
			shouldsleep = 0;	/* We have a command response */
		else if (priv->cur_cmd)//当前还有命令在处理
			shouldsleep = 1;	/* Can't send a command; one already running */
		else if (!list_empty(&priv->cmdpendingq))//命令挂起链表有命令链入，需要处理
	...
	/*	lbs_deb_thread("2: currenttxskb %p, dnld_send %d\n",
		...
		if (priv->resp_len[resp_idx]) {//处理网卡响应
		//	spin_unlock_irq(&priv->driver_lock);
			lbs_process_command_response(priv,
				priv->resp_buf[resp_idx],
				priv->resp_len[resp_idx]);
			//spin_lock_irq(&priv->driver_lock);
			priv->resp_len[resp_idx] = 0;//完成清0
		}
		//spin_unlock_irq(&priv->driver_lock);

		/* command timeout stuff */
		if (priv->cmd_timed_out && priv->cur_cmd) {//命令超时
			...
			if (++priv->nr_retries > 0) {
				...
				lbs_complete_command(priv, cmdnode, -ETIMEDOUT);//超过重试次数直接挂掉
				priv->nr_retries = 0;
				/*if (priv->reset_card)
				...
				list_add(&cmdnode->list, &priv->cmdpendingq);//重新链入处理
			}
		}
		...
		//spin_lock_irq(&priv->driver_lock);
	 /*	while (__kfifo_len(priv->event_fifo)) {//之前提到的envent，包括掉线，热插拔等事件的处理
				u32 event;
			__kfifo_get(priv->event_fifo,(unsigned char *)&event,sizeof(event));
			//spin_unlock_irq(&priv->driver_lock);
			lbs_process_event(priv, event);//处理envent事件
			//spin_lock_irq(&priv->driver_lock);
		} */
		//spin_unlock_irq(&priv->driver_lock);
		...
		if (!priv->dnld_sent && !priv->cur_cmd)//执行下一个命令
			lbs_execute_next_command(priv);//会调用硬件相关的函数将命令写入芯片
		...
		//spin_lock_irq(&priv->driver_lock);
		if (!priv->dnld_sent && priv->tx_pending_len > 0) {//发送数据处理
...
struct lbs_private *lbs_add_card(void *card)
{
	...
	//哪里记住这个priv是个bug
	 if (lbs_init_adapter(priv)) {//初始化lbs_private中的信息，它是管理网卡控制器的主要数据结构
